...
def sobre():
    ...
    ponto_central = Point(350, 50)
    texto = Text(
        ponto_central, "Criado por Ruaneri Ferreira Portela \n FURG Nº 149369 \n 2021")
    texto.draw(win)
    win.getMouse()
    win.close()
...
def ajuda():
    ...
    texto1 = Text(
        ponto_texto1, "Para jogar insira a operaçao que você desconfie que esteja sendo usada na equação, ela pode ser:")
    texto1.draw(win)
    texto2 = Text(ponto_texto2, "(+, -, * , /)")
    texto2.draw(win)
    texto3 = Text(
        ponto_texto3, "Insira o que você acha no primeiro espaço da equerda para a direita e assim sucessivamente")
    texto3.draw(win)
    texto4 = Text(
        ponto_texto4, "Para encerrar o jogo aperte (Esc), Para sair da tela de ajuda aperte na tela")
    texto4.draw(win)
    win.getMouse()
    win.close()
...
def principal():
    ...
    while gat == True:
        try:
            ...
            string_eval = str(valor1)+str(simbolo1)+str(valor2) + \
                str(simbolo2)+str(valor3)+str(simbolo3)+str(valor4)
    ...
    while inicio_logica == True:
        ...
        while l1 == True:
            ...
            if entrada_in_box == simbolo1:
                novoeq = (valor1, simbolo1, valor2, "------",
                          valor3, "-----", valor4, "=", resultado)
        ...
        while l2 == True:
            ...
            if entrada_in_box == simbolo2:
                novoeq = (valor1, simbolo1, valor2, simbolo2,
                          valor3, "-----", valor4, "=", resultado)
                equacao_oculta.setText(novoeq)
                retoro.setText("Acertou(Aperte para descartar!)")
                win.getMouse()
        ...
        while l3 == True:
            ...
            if entrada_in_box == simbolo3:
                novoeq = (valor1, simbolo1, valor2, simbolo2,
                          valor3, simbolo3, valor4, "=", resultado)
                equacao_oculta.setText(novoeq)
                retoro.setText("Acertou(Aperte para descartar!)")
                win.getMouse()
                l3 = False
